$(function(){
	...
	$('#comments button').on('click',function(event){
		...
		$.ajax({
			...
			data:{'comment[textbook]':$('#comments input:eq(0)').val(),//电影ID
			...
			if(data.reply.length){
				...
				$('#mediaBody').append('<div class="media"><div class="pull-left"><img src="/upload/'+data.reply[len-1].from.image+'" style="width: 30px; height: 30px;" /></div><div class="media-body"><h4 class="media-heading">'+data.reply[len-1].from.name+'<span>&nbsp;REPLY&nbsp;</span>'+data.reply[len-1].to.name+'</h4><p>'+data.reply[len-1].content+'</p><span class="createAt">'+format(new Date())+'</span>&nbsp;&nbsp;&nbsp;&nbsp;<a class="comment" href="#comments" data-cid='+data._id+' data-tid='+data.from._id+'>REPLY</a>&nbsp;|&nbsp;<a class="commentDel" href="javascript:;" data-cid='+data._id+' data-did='+data.reply[len-1]._id+'>DELETE</a></div></div>');
			}else{
				$('#mediaList').append('<li class="media"><div class="pull-left"><img src="/upload/'+data.from.image+'" style="width: 40px; height: 40px;" /></div><div class="media-body"><h4 class="media-heading">'+data.from.name+'</h4><p>'+data.content+'</p><span class="createAt">'+format(new Date())+'</span>&nbsp;&nbsp;&nbsp;&nbsp;<a class="comment" href="#comments" data-cid='+data._id+' data-tid='+data.from._id+'>REPLY</a>&nbsp;|&nbsp;<a class="commentDel" href="javascript:;" data-cid='+data._id+'>DELETE</a></div><hr></li>');
			}
